      ...
      MongoClient.connect(
        ...
        function (err, db) {
          ...
          dbo
            ...
            .updateOne(
              whereStr,
              { $set: { applicationname: obj.applicationname } },
              function (err, result) {
                if (err) {
                  return err;
                } else {
              ...
              .updateOne(
                ...
                function (err, result) {
                  if (err) {
                    ...
                    const filename = obj.cangkuname;
                    if (obj.platform.toLowerCase() === "h5") {
                      deleteFolder("../../h5/" + filename);
                    } else {
...
const editcoverage = async (ctx, next) => {
  try {
    ...
    const resultdata = await editcoveragedata(req.myobj).then((data) => {
      return data;
    });
    // console.log('req', req)
    if (resultdata) {
...
const getAllcoverage = async (ctx, next) => {
  try {
    ...
    const req = ctx.request.body;
    const resultdata = await getdatafromdb(req.limitnum).then(
      //dbname,limitnum
      (data) => {
        return data;
      }
    );
    const resultlength = await getdatafromdbinit().then((data) => {
      return data;
    });
...
const searchcoverage = async (ctx, next) => {
  try {
    const req = ctx.request.body;
    const resultdata = await searchfromdb(req.casename, req.limitnum).then(
      // ArrayList
      (data) => {
        return data;
      }
...
const tooglecoverage = async (ctx, next) => {
  try {
    const req = ctx.request.body;
    const resultdata = await switchcoverage(req.myobj).then(
      // ArrayList
      (data) => {
        return data;
      }
...
const getnewcoverage = (obj) => {
  return new Promise((res, rej) => {
    try {
      if (JSON.parse(obj.isSelected)) {
        var productname = obj.applicationname;
        ...
        //        lastcdfile = cdinfile
        //   } else {
        //   lastcdfile =  productname[len - 2]
        //       }
        console.log("start git pull");
        //	var cmdStr = 'git clone -b' + ' ' + obj.branch + ' ' + obj.applicationname

        // var cmdStr = 'cd ' + ' /data1/newproserver/' + productname + ' &&  ' + 'git pull '
        if (obj.platform.toLowerCase() === "h5") {
          var cmdStr =
            "cd /data1/hserver/" + productname + " &&  " + "git pull ";
        } else {
          var cmdStr =
            "cd  /data1/newproserver/" + productname + " &&  " + "git pull ";
        }
        exec(cmdStr, function (err, stdout, stderr) {
          if (err) {
            console.log("open fail:" + stderr);
...
const getnewfile = async (ctx, next) => {
  try {
    const req = ctx.request.body;
    const resultdata = await getnewcoverage(req.myobj).then((data) => {
      return data;
    });
    if (resultdata) {
